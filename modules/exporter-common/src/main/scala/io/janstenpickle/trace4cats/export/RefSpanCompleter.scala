...
object RefSpanCompleter {
  def apply[F[_]: Sync](serviceName: String): F[RefSpanCompleter[F]] = apply[F](TraceProcess(serviceName))
  def unsafe[F[_]: Sync](serviceName: String): RefSpanCompleter[F] = unsafe[F](TraceProcess(serviceName))

  def apply[F[_]: Sync](process: TraceProcess): F[RefSpanCompleter[F]] =
