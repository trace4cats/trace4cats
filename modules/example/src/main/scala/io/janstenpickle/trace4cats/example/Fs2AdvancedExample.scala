...
object Fs2AdvancedExample extends IOApp {
  ...
  // evaluation
  def doTracedWork[F[_]: Sync: Parallel: Temporal: Trace](stream: TracedStream[F, String]): TracedStream[F, Unit] =
    stream.evalMap { time =>
      runF[F](time)
    }
  ...
  def continue[F[_]: MonadCancelThrow, G[_]: Applicative: Defer: Trace](
    ep: EntryPoint[F],
