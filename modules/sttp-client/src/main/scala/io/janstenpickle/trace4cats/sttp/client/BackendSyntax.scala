...
trait BackendSyntax {
  ...
  implicit class TracedBackendSyntax[F[_], S, WS_HANDLER[_]](backend: SttpBackend[F, S, WS_HANDLER]) {
    def liftTrace[G[_]](toHeaders: ToHeaders = ToHeaders.all, spanNamer: SttpSpanNamer = SttpSpanNamer.methodWithPath)(
      implicit
      P: Provide[F, G, Span[F]],
      F: Bracket[F, Throwable],
      G: MonadError[G, Throwable]
    ): SttpBackend[G, S, WS_HANDLER] = new BackendTracer[F, G, S, WS_HANDLER, Span[F]](
      backend,
      Lens.id,
