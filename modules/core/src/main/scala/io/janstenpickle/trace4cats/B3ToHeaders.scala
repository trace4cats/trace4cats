...
private[trace4cats] class B3ToHeaders extends ToHeaders {
  ...
  override def toContext(headers: Map[String, String]): Option[SpanContext] =
    (headers.get(traceIdHeader), headers.get(spanIdHeader), headers.get(parentSpanIdHeader), headers.get(sampledHeader)) match {
      case (Some(traceIdHex), Some(spanIdHex), parentSpanIdHex, sampled) =>
        for {
          ...
          SpanContext(traceId, spanId, parentSpanIdHex.flatMap { hex =>
            SpanId.fromHexString(hex).map(parent => Parent(parent, isRemote = true))
          }, TraceFlags(b3SampledFlag(sampled)), TraceState.empty, isRemote = true)
      case _ => None
    }
